...
//----------------------------------------------------------------------------
#include <include.h>
#include "../libs/vkdbfdf.h"
#include "../libs/vkdbfex.h"
...
_i main(vd){
   ...
   if(dbOpen (2,"test01.dbf")>0){
      ...
      printf ("Количество записей в БД:%ld\n", dbHeaw[2].RecCount);
      printf ("Длинна одной записи:%d\n", dbHeaw[2].RecLen);
      printf ("Номер текущей записи:%ld\n", dbHeaw[2].CRec);
      printf ("Смещение начала данных:%ld\n", dbHeaw[2].COfs);
      printf ("Количество полей в БД:%d\n", dbHeaw[2].FieldCount);
      printf ("-----------------------------------------------------------\n");
      printf ("Время начала записи: %06ld\n",dbCurDate('T'));
      ...
      printf ("Время конца  записи: %06ld\n",dbCurDate('T'));
      printf ("-----------------------------------------------------------\n");
      printf ("Количество записей в БД:%ld\n", dbHeaw[2].RecCount);
      printf ("Номер текущей записи:%ld\n", dbHeaw[2].CRec);
      printf ("-----------------------------------------------------------\n");
